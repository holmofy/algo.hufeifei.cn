...
define([], function () {
    return function (frame) {
        ...
        .after(600, function () {
            frame.model().subtitle = '<h3>对于本例，您可以将<span style=“color:steelblue”>节点</span>视为存储单个值的数据库服务器。</h3>'
                           + frame.model().controls.html();
            layout.invalidate();
        })
        .after(100, wait).indefinite()
        ...
        .after(1000, function () {
            frame.model().subtitle = '<h3>我们还有一个<span style="color:green">客户端</span>，可以向服务器发送值。</h3>'
                           + frame.model().controls.html();
            layout.invalidate();
        })
        .after(500, function () {
        ...
        .after(100, wait).indefinite()
        .after(100, function () {
            frame.model().subtitle = '<h3>通过一个节点，就这个值达成一致或达成<em>共识</em>是很容易的。</h3>'
                           + frame.model().controls.html();
            layout.invalidate();
        })
        .after(100, wait).indefinite()
