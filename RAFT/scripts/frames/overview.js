...
define(["../model/log_entry"], function (LogEntry) {
    return function (frame) {
        ...
        .after(100, function () {
            ...
            model().subtitle = '<h2><em>Candidate(候选者)</em>状态</h2>'
                           + model().controls.html();
            layout.invalidate();
        })
        .after(100, wait).indefinite()
        ...
        .after(300, function () {
            ...
            model().zoom(null);
            node("b")._state = "follower";
            model().subtitle = '<h2>我们所有的节点都以追随者状态开始。</h2>'
                           + model().controls.html();
            layout.invalidate();
        })
        .after(100, wait).indefinite()
        .after(100, function () {
            frame.snapshot();
            model().subtitle = '<h2>如果追随者没有收到领导者的消息，他们就可以成为候选者。</h2>'
                           + model().controls.html();
            layout.invalidate();
        })
        .after(100, function () {
        ...
        .after(100, function () {
            frame.snapshot();
            model().subtitle = '<h2>然后，候选者请求其他节点为自己投票。</h2>'
                           + model().controls.html();
            layout.invalidate();
        })
        .after(100, function () {
        ...
        .after(100, function () {
            frame.snapshot();
            model().subtitle = '<h2>节点对其投票进行回复。</h2>'
                           + model().controls.html();
            layout.invalidate();
        })
        .after(300, function () {
        ...
        .after(100, function () {
            frame.snapshot();
            model().subtitle = '<h2>如果候选者获得大多数节点的投票，则该候选者将成为领导者。</h2>'
                           + model().controls.html();
            layout.invalidate();
        })
        .after(100, wait).indefinite()
        .after(100, function () {
            frame.snapshot();
            model().subtitle = '<h2>这一过程被称为<em>Leader Election(领导者选举)</em>。</h2>'
                           + model().controls.html();
            layout.invalidate();
        })
        .after(100, wait).indefinite()
        ...
        .after(100, function () {
            frame.snapshot();
            model().subtitle = '<h2>现在，系统的所有更改都将通过领导者进行。</h2>'
                           + model().controls.html();
            layout.invalidate();
        })
        .after(100, wait).indefinite()
        ...
        .after(100, function () {
            frame.snapshot();
            model().subtitle = '<h2>每个更改都作为一个条目添加到节点的日志中。</h2>'
                           + model().controls.html();
            layout.invalidate();
        })
        .after(100, wait).indefinite()
        .after(100, function () {
            frame.snapshot();
            model().subtitle = '<h2>此日志项当前未提交，因此不会更新节点的值。</h2>'
                           + model().controls.html();
            layout.invalidate();
        })
        .after(100, wait).indefinite()
        .after(300, function () {
            ...
            model().subtitle = '<h2>要提交日志项，节点首先将其复制到追随者节点...</h2>'
                           + model().controls.html();
            layout.invalidate();
        })
        .after(100, wait).indefinite()
        .after(100, function () {
            ...
            model().subtitle = '<h2>领导者则等到大多数节点已经写入该日志项</h2>'
                           + model().controls.html();
            layout.invalidate();
        })
        .after(1000, function () {
        ...
        .after(100, function () {
            frame.snapshot();
            model().subtitle = '<h2>该日志项现在已在Leader节点上提交，节点状态为“5”.</h2>'
                           + model().controls.html();
            layout.invalidate();
        })
        .after(100, wait).indefinite()
        .after(100, function () {
            ...
            model().subtitle = '<h2>然后，领导者通知追随者该日志项已提交。</h2>'
                           + model().controls.html();
            layout.invalidate();
        })
        .after(100, wait).indefinite()
        .after(100, function () {
            frame.snapshot();
            model().subtitle = '<h2>集群现在已经就系统状态达成共识。</h2>'
                           + model().controls.html();
            layout.invalidate();
        })
        .after(100, wait).indefinite()
        ...
        .after(300, function () {
            frame.snapshot();
            model().subtitle = '<h2>此过程称为<em>Log Replication(日志复制)</em>。</h2>'
                           + model().controls.html();
            layout.invalidate();
        })
        .after(100, wait).indefinite()
